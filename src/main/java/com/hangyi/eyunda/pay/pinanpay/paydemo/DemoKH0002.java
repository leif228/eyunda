...
public class DemoKH0002 {
	public static void main(String[] args) throws CsiiException {
		...
		input.put("beginDate", "20140501000000"); // 查询开始时间（支付完成时间）YYYYMMDDHHMMSS
		input.put("endDate", "20140522240000"); // 查询结束时间（支付完成时间）YYYYMMDDHHMMSS

		try {
			output = util.execute(input, "KH0002"); // 执行发送（KH0002）订单列表信息查询请求，并返回结果对象
			...
			if ((errorCode == null || errorCode.equals("")) && (errorMsg == null || errorMsg.equals(""))) {
				...
				com.ecc.emp.data.IndexedCollection orderList = (com.ecc.emp.data.IndexedCollection) output
						.getDataElement("iOrderListDetail");
				for (int i = 0; i < orderList.size(); i++) {
					com.ecc.emp.data.KeyedCollection orderDetail = (com.ecc.emp.data.KeyedCollection) orderList
							.getElementAt(i);
