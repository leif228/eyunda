...
@Repository
public class HyqWalletSettleDao extends PageHibernateDao<HyqWalletSettle, Long> {

	public HyqWalletSettle getTodayLastOne(Long userId) {
		Criteria criteria = getSession().createCriteria(entityClass);

		criteria.add(Restrictions.eq("userId", userId));

		Page<HyqWalletSettle> page = new Page<HyqWalletSettle>();
		page.setPageSize(1);
		page.setPageNo(1);
		...
		List<HyqWalletSettle> settles = page.getResult();
	...
	public Double sumTodayTrade(Long userId) {
		...
		List<HyqWalletSettle> settles = super.find(criteria);

		Double todayFetchMoney = 0.00D;
		for (HyqWalletSettle settle : settles) {
	...
	public Page<HyqWalletSettle> findByUserId(Long userId, int pageNo, int pageSize, String startTime, String endTime) {
		Page<HyqWalletSettle> page = new Page<HyqWalletSettle>();
	...
	public Page<Long> findUserByTerm(Long userId, RecentPeriodCode termCode, int pageNo, int pageSize) {
		...
		String hql = "select distinct y.userId from HyqWalletSettle y where y.userId > 0";
	...
	public List<HyqWalletSettle> findSettleByUserId(Long userId, RecentPeriodCode termCode) {
		String hql = "select y from HyqWalletSettle y where y.userId = " + userId;
