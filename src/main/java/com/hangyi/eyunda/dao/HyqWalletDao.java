...
@Repository
public class HyqWalletDao extends PageHibernateDao<HyqWallet, Long> {

	public Page<HyqWallet> findWalletByStyle(Long userId, SettleStyleCode settleStyle, PayStatusCode paymentStatus,
			ApplyReplyCode refundStatus, String startTime, String endTime, int pageNo, int pageSize) {
		Page<HyqWallet> page = new Page<HyqWallet>();
	...
	public Page<HyqWallet> findByRefundStatus(ApplyReplyCode refundStatus, Long userId, String startTime,
	...
	public Page<Long> findByKhsdErrorStatus(YesNoCode errorStatus, Long userId, String startTime, String endTime,
		...
		String hql = " select distinct a.id from HyqWallet a, YydWalletKhsd b where a.paymentNo = b.paymentNo";
	...
	public Page<Long> findByZjjzErrorStatus(YesNoCode errorStatus, Long userId, String startTime, String endTime,
		...
		String hql = " select distinct a.id from HyqWallet a, YydWalletZjjz b where a.paymentNo = b.paymentNo";
	...
	public Page<HyqWallet> findByUserId(Long userId, String startTime, String endTime, int pageNo, int pageSize) {
		Page<HyqWallet> page = new Page<HyqWallet>();
		page.setPageNo(pageNo);
		page.setPageSize(pageSize);
		page.setOrder(Page.DESC);
	...
	public HyqWallet findByPaymentNo(String paymentNo) {
		Criteria criteria = this.getSession().createCriteria(entityClass);
		criteria.add(Restrictions.eq("paymentNo", paymentNo));

		List<HyqWallet> yydWallets = super.find(criteria);
	...
	public HyqWallet findByAngel(Long receiverId, Long cabinId) {
		Criteria criteria = this.getSession().createCriteria(entityClass);
	...
	public HyqWallet findByFreight(Long freightId, FeeItemCode feeItem, Long userId) {
		Criteria criteria = this.getSession().createCriteria(entityClass);

		criteria.add(Restrictions.ne("orderId", 0L));
		criteria.add(Restrictions.eq("orderId", freightId));
		criteria.add(Restrictions.eq("feeItem", feeItem));
		criteria.add(Restrictions.eq("buyerId", userId));

		List<HyqWallet> wallets = super.find(criteria);
	...
	public List<HyqWallet> findByOrderId(FeeItemCode feeItem, Long orderId) {
		Criteria criteria = this.getSession().createCriteria(entityClass);
	...
	public List<HyqWallet> getExpiredBills() {
		String hql = " from HyqWallet where settleStyle = " + SettleStyleCode.pay.ordinal() + " and (feeItem = "
		...
		List<HyqWallet> yydWallets = super.find(hql);

		return yydWallets;
	}

	public Page<HyqWallet> findByPayStatus(PayStatusCode payStatus, String startTime, String endTime, int pageNo,
			int pageSize) {
		Page<HyqWallet> page = new Page<HyqWallet>();
		page.setPageNo(pageNo);
		page.setPageSize(pageSize);
	...
	public List<HyqWallet> findByKhWaitStatus(YesNoCode waitStatus) {
		Criteria criteria = this.getSession().createCriteria(entityClass);
