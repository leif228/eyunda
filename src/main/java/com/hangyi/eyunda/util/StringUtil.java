...
public class StringUtil {
	...
	/**
	 * Check site ID Session
	 * 
	 * @param request
	 *            HttpServletRequest
	 * @return boolean true=session has siteid ,false=session do not have siteid
	 */
	public static boolean checkSiteidSession(HttpServletRequest request) {
		boolean boltag = true;
		HttpSession session = request.getSession();
		String site = (String) session.getAttribute("SESSION_SITEID");
	...
	public static boolean isNumeric(String s) {
		...
		if (s != null && !s.equals("")) {
			...
			for (int i = 0; i < numbers.length; i++) {
				if (!Character.isDigit(numbers[i]) && point != numbers[i]) {
					flag = false;
					break;
				}
			}
	...
	/**
	 * replace the specified character to it's ascii code eg. '#'->'$35$' (35 is
	 * '#' 's Ascii code)
	 * 
	 * 
	 * @param sString
	...
	public static String secureWord(String orgin) {
	...
	public static String formatFourNum(int num) {
		int hour = num / 60;
		int min = num - hour * 60;
		String hourStr = "";
		String minStr = "";
