...
public class CalendarUtil {
	...
	public static String toHumanFormat(Calendar date) {
		...
		if (days < -1) {
			if (CalendarUtil.getYear(now) == CalendarUtil.getYear(date))
				d = new SimpleDateFormat("MM-dd").format(date.getTime());
			else
			...
			d = "昨天";
		} else if (days > 1 && days < wNow) {
			int wDate = date.get(Calendar.DAY_OF_WEEK);
			wDate--;
			switch (wDate) {
			case 0:
				d = "周日";
