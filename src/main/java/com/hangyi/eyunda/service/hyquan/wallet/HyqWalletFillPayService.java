...
import com.hangyi.eyunda.service.wallet.ZjjzApiService;
...
public class HyqWalletFillPayService {
	...
	public PayKhsdCode doNotify(YesNoCode khFlag, String sign, String orig) throws Exception {
		...
		if (result) {
			...
			// 更改订单状态、合同状态、支付状态
			HyqWalletData walletData = walletService.getByPaymentNo((String) output.getDataValue("orderId"));
			// 避免重复处理
	...
	// 确认付款
	public boolean confirmBona(HyqWalletData walletData) throws Exception {
		boolean flag = false;
		try {
			HyqWallet wallet = walletDao.get(walletData.getId());
			if (wallet != null) {
				// 付款方付钱到收款方 （如果为合同支付且船东不是船代）
				flag = this.api6070(walletData);
				if (flag) {
					// 更改支付状态
					this.updateWalletStatus(walletData.getId(), PayStatusCode.TRADE_FINISHED, ApplyReplyCode.noapply,
							null, Calendar.getInstance(), null, null, null);
					// 钱包帐务添加收款人直接收款记录
					walletSettleService.saveWalletSettle(walletData.getSellerId(), walletData.getId(),
							WalletSettleCode.receivefee, NumberFormatUtil.format(
									walletData.getTotalFee() - walletData.getMiddleFee() - walletData.getServiceFee()));
				}
			} else {
	...
	// 确认付款
	public boolean confirmPay(HyqWalletData walletData) throws Exception {
		boolean flag = false;
		try {
			HyqWallet wallet = walletDao.get(walletData.getId());
			if (wallet != null) {
	...
	// 退款申请
	public boolean refundApply(HyqWalletData walletData) throws Exception {
		try {
			if (walletData != null) {
	...
	// 退款诚意金或担保金
	public boolean refundBonaOrGu(HyqWalletData walletData) throws Exception {
		boolean flag = false;
		try {
			if (walletData != null) {
	...
	// 退款处理
	public boolean doRefund(HyqWalletData walletData, ApplyReplyCode applyReply, String khRefundNotifyUrl)
			throws Exception {
		boolean flag = false;
		try {
			if (walletData != null) {
	...
	// 调用支付接口上送字段(非绑定支付)
	private KeyedCollection getPayInputOrig(HyqWalletData walletData) {
	...
	// 非协议支付 orig 和 sign
	public Map<String, String> noBindPay(HyqWalletData walletData) {
		Map<String, String> map = new HashMap<String, String>();
		try {
	...
	private KeyedCollection getPayInputOrig(HyqWalletData walletData, String plantBankId) {
		KeyedCollection inputOrig = new KeyedCollection("inputOrig");
		if (walletData != null) {
			inputOrig.put("masterId", Constants.CUSTOMER_NUM); // 商户号，注意生产环境上要替换成商户自己的生产商户号
			inputOrig.put("orderId", walletData.getPaymentNo()); // 订单号，严格遵守格式：商户号+8位日期YYYYMMDD+8位流水
	...
	// 绑定并支付 orig 和 sign
	public Map<String, String> bindAndPay(HyqWalletData walletData, String plantBankId) {
		Map<String, String> map = new HashMap<String, String>();
		try {
			KeyedCollection signDataput = new KeyedCollection("signDataput");
			PayclientInterfaceUtil util = new PayclientInterfaceUtil();
	...
	private KeyedCollection getPayInputOrig(HyqWalletData walletData, String bindId, String verifyCode, String dataTime,
			String notifyurl) {
	...
	// 绑定快捷支付接口调用，要看接口文档
	public PayKhsdCode bindFastPay(HyqWalletData walletData, String bindId, String verifyCode, String dataTime,
	...
	private void qingfenOrSurety(HyqWalletData walletData) throws Exception {
		if (PayStatusCode.WAIT_PAYMENT != walletData.getPaymentStatus())
			return;
	...
	// 获取绑定支付短信验证码上送字段
	private KeyedCollection getValidCodeInput(HyqWalletData walletData, String bindId) {
		KeyedCollection input = new KeyedCollection("input");
		if (walletData != null) {
			input.put("masterId", Constants.CUSTOMER_NUM); // 商户号，注意生产环境上要替换成商户自己的生产商户号
	...
	// API006获取短信验证码时间
	public String getValidCode(HyqWalletData walletData, String bindId) {
		try {
			PayclientInterfaceUtil util = new PayclientInterfaceUtil(); // 建立客户端实例
	...
	// 取得用户绑定的银行卡列表
	public List<HyqUserPinganBankData> getBindCards(HyqWalletData walletData) {
		List<HyqUserPinganBankData> userBankDatas = new ArrayList<HyqUserPinganBankData>();
		try {
			PayclientInterfaceUtil util = new PayclientInterfaceUtil(); // 建立客户端实例
			...
			if ((errorCode == null || errorCode.equals("")) && (errorMsg == null || errorMsg.equals(""))) {
				...
				for (int i = 0; i < bindCards.size(); i++) {
					HyqUserPinganBankData bankData = new HyqUserPinganBankData();
	...
	// 跨行退款input
	private KeyedCollection getRefundInput(HyqWalletData walletData, String notifyurl) {
		KeyedCollection input = new KeyedCollection("input");
		if (walletData != null) {
			input.put("masterId", Constants.CUSTOMER_NUM); // 商户号，注意生产环境上要替换成商户自己的生产商户号
	...
	// 跨行收单支付退款
	public boolean khRefund(HyqWalletData walletData, String notifyurl) throws Exception {
		try {
			PayclientInterfaceUtil util = new PayclientInterfaceUtil();
	...
	// 单笔订单状态查询接口
	private KeyedCollection getKhOrderStatusInput(HyqWalletData walletData) {
		KeyedCollection input = new KeyedCollection("input");
		if (walletData != null) {
			input.put("masterId", Constants.CUSTOMER_NUM); // 商户号，注意生产环境上要替换成商户自己的生产商户号
			input.put("orderId", walletData.getPaymentNo()); // 订单号，严格遵守格式：商户号+8位日期YYYYMMDD+8位流水
	...
	// KH0001 跨行收单单笔订单查询
	public boolean KhOrderStatusQuery(HyqWalletData walletData) throws Exception {
		try {
			PayclientInterfaceUtil util = new PayclientInterfaceUtil();
			// 组装
			...
			if ("1".equals(settleflg)) {
				...
				/*
				 * // 改khsdStatus状态为支付失败 this.updateWalletStatus(walletData.getId(),
				 * PayStatusCode.TRADE_CLOSED, null, null, null, null, YesNoCode.yes,
				 * YesNoCode.no); // 更新合同状态 if (walletData.getFeeItem() == FeeItemCode.prefee)
				 * this.updateOrderStatus(walletData.getBuyerData(), walletData.getOrderId(),
				 * OrderStatusCode.refund, Calendar.getInstance());
				 */
	...
	// 使用钱包支付
	public boolean walletPay(HyqWalletData walletData, String serialNo, String messageCode) throws Exception {
		if (PayStatusCode.WAIT_PAYMENT != walletData.getPaymentStatus())
			throw new Exception("该订单已支付过！");
	...
	// pinganpay会员交易，没用上
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public boolean api6006(HyqWalletData walletData, String funcFlag, HyqAccountData payAccountData,
			HyqAccountData rcvAccountData) throws Exception {
		/* 定义报文参数字典 */
	...
	// 冻结API6007
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public boolean api6007(HyqWalletData walletData, String funcFlag, String serialNo, String messageCode)
			throws Exception {
		/* 定义报文参数字典 */
		HashMap parmaKeyDict = new HashMap();// 用于存放生成向银行请求报文的参数
	...
	// 登记挂账API6008，没用上
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public boolean api6008(HyqWalletData walletData, HyqAccountData accountData) throws Exception {
		/* 定义报文参数字典 */
		HashMap parmaKeyDict = new HashMap();// 用于存放生成向银行请求报文的参数
	...
	// 非验密会员交易API6034
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public boolean api6034(HyqWalletData walletData, String funcFlag, String serialNo, String messageCode)
			throws Exception {
		/* 定义报文参数字典 */
		HashMap parmaKeyDict = new HashMap();// 用于存放生成向银行请求报文的参数
	...
	// 清分API6056
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public boolean api6056(HyqWalletData walletData) throws Exception {
		/* 定义报文参数字典 */
		HashMap parmaKeyDict = new HashMap();// 用于存放生成向银行请求报文的参数
	...
	// 会员资金支付 api6070
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public boolean api6070(HyqWalletData walletData) throws Exception {
		/* 定义报文参数字典 */
		HashMap parmaKeyDict = new HashMap();// 用于存放生成向银行请求报文的参数
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private Map<String, Object> api6082(HyqWalletData walletData, String tranType) throws Exception {
		/* 定义报文参数字典 */
		HashMap parmaKeyDict = new HashMap();// 用于存放生成向银行请求报文的参数
		HashMap retKeyDict = new HashMap();// 用于存放银行发送报文的参数
		...
		/* 报文参数赋值 */
		if (walletData != null) {
			HyqAccountData payAccountData = accountService.getAccountByUserId(walletData.getBuyerId(),
	...
	public HyqWalletData getWalletData(Long userId, Long walletId, Long orderId, FeeItemCode feeItem, Double payMoney,
	...
	private HyqWalletData createWalletData(Long userId, Long orderId, FeeItemCode feeItem, Double payMoney,
			Integer suretyDays, String remark) throws Exception {
		Long walletId = 0L;
		...
		case bona:
			// 运单诚意金（freightId）
			// walletId = this.bonaWalletData(userId, orderId, payMoney, suretyDays,
			// remark);
			break;
		case guarantee:
			// 运单担保金（freightId）
			// walletId = this.guaranteeWalletData(userId, orderId, payMoney, suretyDays,
			// remark);
			break;
		case bonus:
			// 发红包（orderId 为收款方id）
	...
	private Long fillWalletData(Long userId, Double payMoney) {
		HyqWallet wallet = new HyqWallet();
	...
	// 合同诚意金支付，在钱包中产生记录
	// private Long bonaWalletData(Long userId, Long freightId, Double payMoney,
	// Integer suretyDays, String remark)
	// throws Exception {
	// ScfFreightData freightData = freightService.getFreightData(freightId);
	//
	// if (freightData != null) {
	//
	// HyqUserData buyer = (freightData.getOwner().getId().equals(userId)) ?
	// freightData.getOwner()
	// : freightData.getMaster();
	// HyqUserData seller = (freightData.getOwner().getId().equals(userId)) ?
	// freightData.getMaster()
	// : freightData.getOwner();
	//
	// HyqWallet wallet = new HyqWallet();
	...
	// wallet.setPaymentNo(paySequenceService.getPaymentNo()); // 随机生成订单号
	// wallet.setFeeItem(FeeItemCode.bona);
	// wallet.setOrderId(freightId);
	//
	// // 卖家其实是财务，诚意金由平台财务暂时保管
	// // wallet.setSellerId(Constants.FINANCE_USERID);
	// // 卖家即潜在的收款方
	// wallet.setSellerId(seller.getId());
	// // 代理，没有
	// wallet.setBrokerId(0L);
	// // 买家其实是付款方，货方及船方交纳诚意金均是付款方
	// wallet.setBuyerId(buyer.getId());
	//
	// Calendar caleTime = CalendarUtil.now();
	...
	// wallet.setMiddleFee(0D); // 代理人佣金
	// wallet.setServiceFee(0D); // 平台收取服务费
	//
	// wallet.setPaymentStatus(PayStatusCode.WAIT_PAYMENT);
	// wallet.setGmtPayment(caleTime);
	...
	// 合同担保金支付，在钱包中产生记录
	// private Long guaranteeWalletData(Long userId, Long freightId, Double
	// payMoney, Integer suretyDays, String remark)
	// throws Exception {
	// ScfFreightData freightData = freightService.getFreightData(freightId);
	//
	// if (freightData != null) {
	...
	// wallet.setSettleStyle(SettleStyleCode.pay);
	// wallet.setPaymentNo(paySequenceService.getPaymentNo()); // 随机生成订单号
	// wallet.setFeeItem(FeeItemCode.guarantee);
	// wallet.setOrderId(freightId);
	//
	// // 卖家，没有
	// wallet.setSellerId(0L);
	// // 代理，没有
	// wallet.setBrokerId(0L);
	// // 买家其实是付款方，货方交纳担保金
	// wallet.setBuyerId(userId);
	...
	private Long faceWalletData(Long senderId, Long receiverId, Double payMoney, Integer suretyDays, String remark) {
		...
		// 买家
		HyqUserData sndUserData = userService.getById(senderId);
		wallet.setBuyerId(senderId);

		// 卖家
		HyqUserData rcvUserData = userService.getById(receiverId);
	...
	private boolean isThreeOrder(HyqWalletData walletData) throws Exception {
		return false;
	}

	public Map<String, String> getSerialNo(HyqWalletData walletData, String tranType) {
		Map<String, String> map = new HashMap<String, String>();
		try {
	...
	public List<ZjjzFetchData> queryUserFetch(Long userId, RecentPeriodCode termCode) throws Exception {
		HyqAccountData payAccountData = accountService.getAccountByUserId(userId, PayStyleCode.pinganpay);
