...
public class HyqChatRoomService extends BaseService<YydChatRoom, Long> {
			...
			if (!crss.isEmpty() && crss.size() == 2) {
				for (YydChatRoomStatus roomStatus : crss) {
					if (!roomStatus.getUserId().equals(userId)) {
						HyqUserData u = userService.getById(roomStatus.getUserId());
	...
	public Long saveChatRoom(ChatRoomData chatRoomData, Long[] userIds) throws Exception {
		...
		if (uids != null && uids.size() > 0) {
			...
			for (int i = 0; i < uids.size(); i++) {
				HyqUserData u = userService.getById(uids.get(i));
			...
			if (roomStatus != null) {
				...
				if (crss.isEmpty()) {
					...
					for (int i = 0; i < crss.size(); i++) {
						HyqUserData u = userService.getById(crss.get(i).getUserId());
						groupName += u.getTrueName() + ",";
						logoPaths[i] = realPath + u.getUserLogo();// UserData中的userLogo一定不为空
					}
					...
					yydChatRoom.setRecentlyTitle(userService.getById(userId).getTrueName() + "退出了聊天室");
	...
	public List<HyqUserData> getRoomMembers(Long roomId) throws Exception {
		YydChatRoom yydChatRoom = chatRoomDao.get(roomId);

		List<HyqUserData> us = new ArrayList<HyqUserData>();

		if (yydChatRoom != null) {
			List<YydChatRoomStatus> statuss = chatRoomStatusDao.getByRoomId(roomId);
			for (YydChatRoomStatus status : statuss) {
				HyqUserData u = userService.getById(status.getUserId());
				us.add(u);
			}
		}
		return us;
