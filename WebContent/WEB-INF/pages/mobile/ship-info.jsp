  ...
  <div id="container" style="margin: 0px auto">
    <div id="order-basic-info" style="margin-top: 5px;">
      <div class="fluid">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <c:choose>
                  <c:when test="${!empty myShipData.broker.userLogo}">
                    <img class="contact-item-object"
                      ...
                      src="${ctx}/download/imageDownload?url=${myShipData.broker.userLogo}"
                      alt="" />
                  </c:when>
                  <c:otherwise>
                    <img class="contact-item-object"
                ...
                <c:choose>
                  <c:when test="${!empty myShipData.broker.trueName}">
                    ${myShipData.broker.trueName}
                  </c:when>
                  <c:otherwise>
                    ${myShipData.broker.nickName}
                  </c:otherwise>
                </c:choose> <br />
                ${myShipData.broker.mobile}
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="font-weight: bold">载重(吨)</td>
              <td>${myShipData.aTons}</td>
            </tr>
            <tr>
              <td style="font-weight: bold">载箱(TEU)</td>
        ...
        <div class="order-title">${templateData.title}合同条款</div>
        <div id="order-more-detail">
          <div class="fluid">
            <c:forEach items="${templateData.tempItemDatas}" var="tempItemData"
              varStatus="getIndex">
              <div style="text-align: left">${tempItemData.no}.${tempItemData.content}</div>
            </c:forEach>
          </div>
        </div>
        <!-- 详情结束 -->
