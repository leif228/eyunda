...
<body>
        ...
        <div class="content">
              ...
              <c:forEach var="bigAreaCode" items="${bigAreaCodes}">
                <c:choose>
                  <c:when test="${bigAreaCode==selectBigAreaCode }">
                    <a class="link"
                      href="${ctx}/portal/home/sortCargoList?bigAreaCode=${bigAreaCode.code}">${bigAreaCode.fullName}</a>&nbsp;&nbsp;
                            </c:when>
                  <c:otherwise>
                    <a
                      href="${ctx}/portal/home/sortCargoList?bigAreaCode=${bigAreaCode.code}">${bigAreaCode.fullName}</a>&nbsp;&nbsp;
          ...
          <div class="content-breadcrumb">
            ...
            <ul class="breadcrumb bold"
              style="font-size: 12px; padding-left: 20px; background: #eee">

              <c:forEach var="bigCargoType" items="${bigCargoTypes}">
                <c:set var="tag" value="t" />
                <c:forEach var="cargoType" items="${bigCargoType.cargoTypes}">
                  <c:if
                    test="${!empty selectSortCargo && selectSortCargo == cargoType && tag == 't' }">
                    <li><a
                      href="${ctx}/portal/home/sortCargoList?cargoTypeCode=${cargoType}&bigAreaCode=${selectBigAreaCode.code}"><span
                        class="color-teal"> <c:if
                            test="${cargoType.cargoBigType != 'container' }">
                        ${fn:substring(cargoType.description, 4, fn:length(cargoType.description))}
                        </c:if> <c:if
                            test="${cargoType.cargoBigType == 'container' }">
                        ${cargoType.cargoBigType.description}
                        </c:if>
                  ...
                  <c:if
                    test="${!empty selectSortCargo && selectSortCargo != cargoType && tag == 't' }">
                    <li><a
                      href="${ctx}/portal/home/sortCargoList?cargoTypeCode=${cargoType}&bigAreaCode=${selectBigAreaCode.code}">
              ...
              <div class="span12">
                ...
                <div class="box corner-all">
                  ...
                  <div class="gallery-controls bottom">
                    ...
                    <div>
                      <form action="${ctx}/portal/home/sortCargoList" id="pageform"
                        method="post">
                        <c:choose>
                          <c:when test="${!empty selectSortCargo}">
                            <input type="hidden" name="cargoTypeCode"
                              value="${selectSortCargo }" />
                            <input type="hidden" id="bigAreaCode" name="bigAreaCode"
  ...
  <script src="${ctx}/js/portal/portal-sortCargoList.js"></script>
  <script type="text/javascript">
    var _rootPath = "${ctx}";
    var pageNo = "${pageData.pageNo}";
    var cargoTypeCode = "${selectSortCargo}";
