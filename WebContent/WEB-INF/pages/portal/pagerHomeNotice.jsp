...
<script>
  function jumpPage(currPageNo) {
    ...
    $("#pageform").attr("action",_rootPath+"/portal/home/notice");
...
<div class="parentDiv">
  <input type="hidden" id="pageNo" name="pageNo" value="${notices.pageNo}" />

  <div class="floatcom pagebody">

    <c:set var="startIndex" value="2" />
    <c:set var="begin" value="${notices.pageNo-startIndex}" />
    <c:set var="end" value="${notices.pageNo+startIndex}" />
    ...
    <c:if test="${end>=notices.totalPages}">
      <c:set var="end" value="${notices.totalPages}" />
    ...
    <ul class="pager helper-font-small">
      <li>共${notices.totalCount}条纪录 页次:${notices.pageNo}/${notices.totalPages}页</li>
      <c:choose>
        <c:when test="${notices.pageNo==1}">
      ...
      <c:choose>
        ...
        <c:otherwise>
          <li><a href="javascript:jumpPage(${notices.pageNo-1})">上一页</a></li>
        </c:otherwise>
      </c:choose>

      <c:forEach begin="${begin}" end="${end}" var="p">
        <c:choose>
          <c:when test="${p==notices.pageNo}">
      ...
      <c:choose>
        <c:when test="${notices.pageNo==notices.totalPages}">
          <li class="disabled"><a href="#">下一页</a></li>
        </c:when>
        <c:otherwise>
          <li><a href="javascript:jumpPage(${notices.pageNo+1})">下一页</a></li>
        </c:otherwise>
      </c:choose>

      <c:choose>
        <c:when test="${notices.totalPages == notices.pageNo}">
          <li class="disabled"><a href="#">尾页</a></li>
        </c:when>
        <c:otherwise>
          <li><a href="javascript:jumpPage(${notices.totalPages})">尾页</a></li>
        </c:otherwise>
      </c:choose>
      <li>转到：
