                ...
                <div class="widget-box">
                  ...
                  <div class="widget-content nopadding">
                    <table class="table table-bordered data-table table-striped">
                      <thead>
                       <tr>
                        <th style="width: 11%">支付编号</th>
                        <th style="width: 8%">帐务类型</th>
                        <th style="width: 11%">交易时间</th>
                        <th style="width: 25%">帐户</th>
                        <th style="width: 25%">交易描述</th>
                      ...
                      <tbody>
                        <c:forEach items="${pageData.result}" var="walletData">
                          <tr>
                            <td>${walletData.shortPaymentNo}</td>
                            <td>${walletData.settleStyle.description}</td>
                            <td>${walletData.gmtPayment}</td>
                            <td>
                              <c:if test="${walletData.settleStyle == 'fill'}">
                                ${walletData.rcvAccountName}${walletData.rcvCardNo}
                              </c:if>
                              <c:if test="${walletData.settleStyle == 'fetch'}">
                                ${walletData.sndAccountName}${walletData.sndCardNo}
                              </c:if>
               	              <c:if test="${walletData.settleStyle == 'pay'}">
                                付款:${walletData.sndAccountName}${walletData.sndCardNo}<br />
                                收款:${walletData.rcvAccountName}${walletData.rcvCardNo}<br />
                                代理:${walletData.brokerAccountName}${walletData.brokerCardNo}
                              </c:if>
                            </td>
	                      	<td>${walletData.body}</td>

                            <%-- <td>
                              <c:if test="${walletData.settleStyle == 'fill'}"></c:if>
                              <c:if test="${walletData.settleStyle == 'fetch'}"></c:if>
                              <c:if test="${walletData.settleStyle == 'pay'}">
                                <c:if test="${userData.id == walletData.buyerData.id}">付款</c:if>
                                <c:if test="${userData.id == walletData.sellerData.id}">收款</c:if>
                                <c:if test="${userData.id == walletData.brokerData.id && userData.id != walletData.buyerData.id && userData.id != walletData.sellerData.id}">佣金</c:if>
                              </c:if>
                            </td>
                            <td style="text-align:right;">
                              <c:if test="${walletData.settleStyle == 'fill'}">${walletData.totalFee}</c:if>
                              <c:if test="${walletData.settleStyle == 'fetch'}">${walletData.totalFee}</c:if>
                              <c:if test="${walletData.settleStyle == 'pay'}">
                                <c:if test="${userData.id == walletData.buyerData.id}">${walletData.totalFee}</c:if>
                                <c:if test="${userData.id == walletData.sellerData.id}">${walletData.totalFee - walletData.middleFee - walletData.serviceFee}</c:if>
                                <c:if test="${userData.id == walletData.brokerData.id && userData.id != walletData.buyerData.id && userData.id != walletData.sellerData.id}">${walletData.middleFee}</c:if>
                              </c:if>
                            </td> --%>
                      	    <td>
                      	      <c:if test="${walletData.settleStyle == 'fetch'}">${walletData.paymentStatus.remark}</c:if>
                      	      <c:if test="${walletData.settleStyle == 'fill'}">
                      	        <c:if test="${walletData.paymentStatus != 'WAIT_PAYMENT'}">${walletData.paymentStatus.remark2}</c:if>
                      	        <c:if test="${walletData.paymentStatus == 'WAIT_PAYMENT' && walletData.khRefundFlag == 'yes'}">充值中...</c:if>
                      	        <c:if test="${walletData.paymentStatus == 'WAIT_PAYMENT' && walletData.khRefundFlag == 'no'}">${walletData.paymentStatus.remark2}</c:if>
                      	      </c:if>
                      	      <c:if test="${walletData.settleStyle == 'pay'}">
                      	        <c:if test="${walletData.paymentStatus != 'WAIT_PAYMENT'}">${walletData.paymentStatus.description}</c:if>
                      	        <c:if test="${walletData.paymentStatus == 'WAIT_PAYMENT' && walletData.khRefundFlag == 'yes'}">支付中...</c:if>
                      	        <c:if test="${walletData.paymentStatus == 'WAIT_PAYMENT' && walletData.khRefundFlag == 'no'}">${walletData.paymentStatus.description}</c:if>
                      	      </c:if>
                      	      <c:if test="${walletData.paymentStatus == 'WAIT_CONFIRM' && walletData.refundStatus != 'noapply'}">
                      	        <br />${walletData.refundStatus.description}
                      	      </c:if>
                      	    </td>
	                      	<td>
